...
// ── Stream settings ───────────────────────────────────────
#define STREAM_QUALITY 5        // JPEG quality 0-63 (lower = better; was 25, now crisp)
...
bool initCamera() {
  ...
  config.frame_size   = FRAMESIZE_SVGA;       // 800x600 (was QVGA 320x240 — PSRAM handles it)
...
void setup() {
  ...
  if (cameraOk) {
    ...
    if (s) {
      // Orientation + base levels
      s->set_vflip(s, 1);           // AI Thinker module is physically upside-down
      s->set_brightness(s, 1);      // slight lift (greenhouse can be dim)
      s->set_saturation(s, 0);      // neutral — AWB handles color balance
      s->set_contrast(s, 0);        // neutral

      // Auto-exposure / gain — handles sun/shade swings in a greenhouse
      s->set_exposure_ctrl(s, 1);   // AEC on
      s->set_gain_ctrl(s, 1);       // AGC on
      s->set_gainceiling(s, GAINCEILING_8X);  // allow more gain in low light

      // White balance — fixes washed-out greens and blue skies
      s->set_whitebal(s, 1);        // AWB on
      s->set_awb_gain(s, 1);        // AWB gain on
      s->set_wb_mode(s, 0);         // 0 = auto

      // Lens correction + gamma — removes corner darkening, evens out tones
      s->set_lenc(s, 1);            // lens correction on
      s->set_raw_gma(s, 1);         // gamma on

      // Defect pixel correction — cleans up hot/dead pixels
      s->set_bpc(s, 1);             // black pixel correction
      s->set_wpc(s, 1);             // white pixel correction

      // Sharpness — slight boost for plant detail (leaves, stems)
      s->set_sharpness(s, 1);

      // Denoise off — preserves detail (OV2640 denoise softens too much)
      s->set_denoise(s, 0);
    }
